...
Route::group(['middleware' => ['admin']], function () {
    ...
    // Clear cache
    Route::post('/clear_cache', array('uses'=>'AdminController@clear_cache'));

    // HTML-Page routes
    Route::get('/manage', array('uses'=>'AdminController@manage_page'));
    Route::get('/instructions', array('uses'=>'AdminController@instructions_page'));

    // JSON endpoints (to be used with AJAX)
    Route::post('/names', array('uses'=>'AdminController@create'));
    Route::post('/names/{nid}/seeding', array('uses'=>'AdminController@node_seeding'))->where('nid','[0-9]+');
    ...
    //Route::post('/load_and_rebuild', array('uses'=>'WebController@load_and_rebuild'));
    Route::post('/load_and_rebuild', array('uses'=>'WebController@load_depth_first'));
...
Route::group(['middleware' => ['logged']], function () {
    ...
    // JSON endpoints (to be used with AJAX)
    Route::get('/tree_roots', array('uses'=>'WebController@tree_roots'));   // To initialize the tree UI
    Route::get('/node_children', array('uses'=>'WebController@node_children')); // Unfold a parent node (the js library requires a get parameter in the URL)
    Route::get('/all_node_children', array('uses'=>'WebController@all_node_children')); // Same as /node_children but includes the not-accepted names
